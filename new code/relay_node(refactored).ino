...
float readEnergyProduction() {
	if (Firebase.getString(firebaseData1, energyProductionPath)) {
		return firebaseData1.stringData().toFloat();
	} 
	else {
...
float readVoltage() {
	if (Firebase.getString(firebaseData1, voltagePath)) {
    return firebaseData1.stringData().toFloat();
	} 
  else{
    return Voltage;
...
void FirebaseReceive() {
	...
	for (int i = 0; i < numRelays; i++) {
      ...
      energyProduction = readEnergyProduction() * 1000;
      Serial.print("Energy Production is: ");
      Serial.println(energyProduction);
